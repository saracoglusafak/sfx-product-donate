...
add_action('admin_menu', 'sfxplugin_admin_menu');

// var_dump($GLOBALS["CONFIGS"]["plugin_name"]);

// echo sfxdonate_images_url;

function sfxplugin_admin_menu()
{

	add_menu_page(
		__($GLOBALS["CONFIGS"]["MENUS"][0]["page_title"], sfxdonate_plugin_id),
		__($GLOBALS["CONFIGS"]["MENUS"][0]["menu_title"], sfxdonate_plugin_id),
	...
	if ($GLOBALS["CONFIGS"]["MENUS"])
		foreach ($GLOBALS["CONFIGS"]["MENUS"] as $k => $v) {
			add_submenu_page(
				$v["parent_slug"],
				__($v["page_title"], sfxdonate_plugin_id),
				__($v["menu_title"], sfxdonate_plugin_id),
				$v["capability"],
				$v["menu_slug"],
				$v["function"]
			);
